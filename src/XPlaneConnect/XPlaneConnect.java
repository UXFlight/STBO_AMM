...
package XPlaneConnect;

import XPlaneConnect.discovery.Beacon;
...
public class XPlaneConnect implements AutoCloseable
{
    ...
    /**
     * Added fun - get ground elev from lat lon
     * @throws IOException 
     */
    public float getAlt(float lat, float lon) throws IOException {
        ...
        ByteBuffer bbReturn = ByteBuffer.wrap(data);
        bbReturn.order(ByteOrder.LITTLE_ENDIAN);

        return bbReturn.getFloat(13);
...
//        return result;
    ...
    public double[] getPOSI(int ac) throws IOException
    {
        ...
        for(int i = 0; i < 7; ++i)
        {
            ...
            /*
        result[0] = bb.getFloat(6);
        result[1] = bb.getFloat(14);
        result[2] = bb.getFloat(22);
        result[3] = bb.getFloat(26);
        result[4] = bb.getFloat(30);
        result[5] = bb.getFloat(34);
        result[6] = bb.getFloat(38);
        */
        return result;
    }
    ...
    public double getTerr(float lat, float lon) throws IOException {
        ByteBuffer bb = ByteBuffer.allocate(8 * 3);
        bb.order(ByteOrder.LITTLE_ENDIAN);
        bb.putDouble((double) lat);
        bb.putDouble((double) lon);
        bb.putDouble(1000.);
    	
    	ByteArrayOutputStream os = new ByteArrayOutputStream();
    	os.write("GETT".getBytes(StandardCharsets.UTF_8));
        os.write(0xFF); //Placeholder for message length
        os.write(0); // ac ID -- not used if all coords are provided
        os.write(bb.array());
        
        if (os.size() != 30) {
        	System.out.println("Get terr buffer size should be 30 and is " + os.size());
        }
    	sendUDP(os.toByteArray());
...
//        {
//        	if (i < 3) {
//        		result[i] = bb2.getDouble(6 + 8 * i);
//        	}
//        	else {
//        		result[i] = bb2.getFloat(6 + 4 * i);
//        	}
//        }
        return bb2.getDouble(22); // only terrain elevation
    }
